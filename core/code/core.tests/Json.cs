...
public class JsonObjectExtensionsTests
{
    ...
    {
        var generator = from propertyName in GenExtensions.GenerateDefault<string>()
    ...
    [Property]
    public Property AddProperty_adds_property_based_on_option()
    {
        var generator = from propertyName in GenExtensions.GenerateDefault<string>()
                        from option in GenExtensions.GenerateDefault<string>().OptionOf()
                        select (propertyName, option);
        ...
        {
            // Arrange
            var (propertyName, option) = input;
            var jsonObject = new JsonObject();

            // Act
            jsonObject = jsonObject.AddProperty(propertyName, option);

            // Assert
            option.Match(value => jsonObject.Should().ContainKey(propertyName).WhoseValue!.GetValue<string>().Should().Be(value),
                         () => jsonObject.Should().NotContainKey(propertyName));
        });
    }
